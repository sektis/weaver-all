...
// 초기 데이터 (옵션)
$initial_data = isset($data) ? $data : array();
$initial_lat = isset($initial_data['lat']) ? $initial_data['lat'] : '';
$initial_lng = isset($initial_data['lng']) ? $initial_data['lng'] : '';
$initial_address = isset($initial_data['address_name']) ? $initial_data['address_name'] : '';
?>

<div id="<?php echo $skin_id; ?>" class="wv-location-address-skin">
    <style>
    ...
    <div class="map-section">
        <div id="kakao-map-<?php echo $skin_id; ?>" class="w-100 h-100"></div>
    ...
    <!-- 선택된 주소 표시 -->
    <div class="address-display empty" id="selected-address-<?php echo $skin_id; ?>">
...
<script>
    (function() {
        ...
        function updateLocationData(lat, lng, region1, region2, region3, fullAddress) {
            ...
            // 주소 표시 업데이트
            var $display = $('#selected-address-' + skinId);
        ...
        function initMap() {
            ...
            var container = document.getElementById('kakao-map-' + skinId);
            var options = {
            ...
            if (currentData.lat && currentData.lng) {
                ...
                if (currentData.address_name) {
                    $('#selected-address-' + skinId).removeClass('empty').text(currentData.address_name);
                }
            }
        }
        ...
        function bindEvents() {
            // 주소 검색
            $wrapper.on('keypress', '.address-search-input', function(e) {
                if (e.which === 13) {
                    e.preventDefault();
            ...
            $wrapper.on('click', '.search-btn', function() {
                var query = $wrapper.find('.address-search-input').val();
                searchAddress(query);
            });

            // 현재 위치 버튼
            $wrapper.on('click', '.current-location-btn', function() {
                getCurrentLocation();
            });
        }
        ...
        window['wv_location_address_' + skinId] = {
